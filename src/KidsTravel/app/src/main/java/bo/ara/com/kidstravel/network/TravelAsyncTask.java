...
public class TravelAsyncTask extends AsyncTask<Travel, Void, Travel> {
    ...
    protected void onPostExecute(Travel travel) {
        ...
        if (travel == null) {
            // Fail
            Toast.makeText(minorsActivity, "Save travel fail", Toast.LENGTH_LONG).show();
        }
        else {
            if(travel.get_id().equals(""))
                Toast.makeText(minorsActivity, "Save travel fail", Toast.LENGTH_LONG).show();
            else{
                Toast.makeText(minorsActivity, "Save travel successful, travelId: " + travel.get_id() + ", travelRoute: " + travel.getTravelRoute(), Toast.LENGTH_LONG).show();
